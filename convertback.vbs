...
With oSheet.Columns("A")
    .ColumnWidth = 15
End With
With oSheet.Columns("B:D")
    .ColumnWidth = 13
End With
With oSheet.Columns("E")
    .ColumnWidth = 65
End With
...
'set header row gray
    ...
    For intRowCounter = 2 to oSheet.usedRange.Rows.Count 'Skip header row
        For intColumnCounter = 1 to oSheet.usedRange.Columns.Count
            If instr(1, oSheet.Cells(intRowCounter, intColumnCounter).Value, "ADDED", 1) > 0 Then
    ...
    For intRowCounter = 2 to oSheet.usedRange.Rows.Count 'Skip header row
        For intColumnCounter = 1 to oSheet.usedRange.Columns.Count
            If instr(1, oSheet.Cells(intRowCounter, intColumnCounter).Value, "DELETED", 1) > 0 Then
    ...
    For intRowCounter = 2 to oSheet.usedRange.Rows.Count 'Skip header row
        For intColumnCounter = 1 to oSheet.usedRange.Columns.Count
            If instr(1, "REPORTDATE|PROJID|PROJNAME|STATENUM|FEDNUM|REGION|LOCATION|PROJMAN|PHONENUM|ANTADVDT|DESCRIPTION|ADDLINFO|ENGEST", oSheet.Cells(intRowCounter, intColumnCounter).Value, 1) > 0 AND intColumnCounter = 2 Then
...
With oSheet.Rows("1:" & oSheet.Rows.Count )
    .RowHeight = 14.40
End With
